...
st.markdown(
    '<div class="sub-title">Application Streamlit pour rechercher les OF, articles, moules et machines</div>',
    unsafe_allow_html=True
)
...
@st.cache_data
def charger_donnees():
    # Chargement des fichiers Excel présents dans le dépôt GitHub
    ...
    # Contrôle qualité : OF du fichier 1 absents du fichier 2
    of_f1 = set(nettoyer_texte(df1["Ordre"]))
    of_f2 = set(nettoyer_texte(df2["N_OF_GPAO"]))
    of_absents_dans_f2 = sorted(list(of_f1 - of_f2))

    return df1, df2, base_recherche, of_absents_dans_f2

# =========================
# Chargement principal
# =========================
try:
    df1, df2, base_recherche, of_absents = charger_donnees()
except FileNotFoundError:
    st.error("Les fichiers Excel ne sont pas trouvés dans le dossier du projet.")
    st.info("Placez les fichiers suivants dans le même dossier que app.py :")
...
articles = sorted([x for x in base_recherche["Code article"].dropna().unique().tolist() if x and x.lower() != "nan"])
machines = sorted([x for x in base_recherche["Machine"].dropna().unique().tolist() if x and x.lower() != "nan"])
moules = sorted([x for x in base_recherche["Moule"].dropna().unique().tolist() if x and x.lower() != "nan"])
...
with tab1:
    ...
    if article_input:
        ...
        result_article = result_article.sort_values(by=["Date", "OF"])
        st.write(f"Résultats : {len(result_article)} ligne(s)")
        st.dataframe(result_article, use_container_width=True)

with tab2:
    st.subheader("Recherche par numéro d’OF")
    of_input = st.text_input("Entrer un numéro d’OF", key="of_input")

    if of_input:
        result_of = base_recherche[
            base_recherche["OF"] == of_input.strip()
        ].drop_duplicates().sort_values(by=["Date"])

        st.write(f"Résultats : {len(result_of)} ligne(s)")
        st.dataframe(result_of, use_container_width=True)
...
with tab3:
    ...
    if moule_input:
        ...
        result_moule = result_moule.sort_values(by=["Machine", "Date"])
        st.write(f"Résultats : {len(result_moule)} ligne(s)")
        st.dataframe(result_moule, use_container_width=True)

with tab4:
    st.subheader("Recherche par machine")
    machine_input = st.text_input("Entrer un numéro de machine", key="machine_input")

    if machine_input:
        result_machine = base_recherche[
            base_recherche["Machine"] == machine_input.strip()
        ][["Machine", "Moule", "OF", "Code article", "Libellé article", "Date"]].drop_duplicates()

        result_machine = result_machine.sort_values(by=["Moule", "Date"])
        st.write(f"Résultats : {len(result_machine)} ligne(s)")
        st.dataframe(result_machine, use_container_width=True)

with tab5:
    st.subheader("Base complète filtrée")
    base_affichage = base_filtre.sort_values(by=["Date", "OF"])
...
with tab6:
    st.subheader("Contrôle qualité des OF")
    if len(of_absents) == 0:
        st.success("Tous les OF du fichier 1 sont présents dans le fichier 2.")
    else:
        st.warning(f"{len(of_absents)} OF du fichier 1 sont absents du fichier 2.")
        st.dataframe(pd.DataFrame({"OF absents du fichier 2": of_absents}), use_container_width=True)

# =========================
# Pied de page
# =========================
st.divider()
st.caption("Application Streamlit générée pour la traçabilité entre OF, articles, moules et machines.")
